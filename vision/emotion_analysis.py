...
def classify_emotion(face_landmarks):
    ...
    soft_smile = (
        ...
        corner_lift > 0.0008 and
        (mouth_height > 0.002 or corner_lift > 0.004)
    )
...
def get_emotion_feedback(score, ratio):
    ...
    if score >= 90:
        return (
            f"발표 내내 밝은 표정을 훌륭하게 유지했어요. "
            f"풍부한 표정이 청중에게 강한 친근감과 신뢰감을 줘요. 지금처럼 계속 유지해봐요!"
        )
    elif score >= 80:
        return (
            f"밝은 표정이 자주 나와요. 발표 분위기가 한층 생동감 있어 보여요. "
            f"강조하고 싶은 내용 앞에서 미소를 더해주면 청중의 집중도를 더 높일 수 있어요."
        )
    elif score >= 70:
        return (
            f"밝은 표정이 잘 나오고 있어요. "
            f"중요한 내용을 말하기 직전에 자연스러운 미소를 더해주면 더 좋아요. 목표는 미소 비율 40% 이상이에요."
        )
    elif score >= 65:
        return (
            f"미소 비율이 목표(30%)에 거의 도달했어요. "
            f"발표 중 숨을 고를 때 입꼬리를 살짝 올리는 습관을 들이면 자연스럽게 늘어나요."
        )
    elif score >= 60:
        return (
            f"밝은 표정이 조금 적어요. 무표정하게 보일 수 있어요. "
            f"발표 전 거울을 보며 미소 짓는 연습을 해보세요. 목표는 미소 비율 30% 이상이에요."
        )
    elif score >= 55:
        return (
            f"밝은 표정이 많이 적어요. 청중에게 다소 딱딱하게 보일 수 있어요. "
            f"입꼬리를 살짝 올리는 것만으로도 인상이 크게 달라져요. 목표는 미소 비율 30% 이상이에요."
        )
    else:
        return (
            f"밝은 표정이 거의 없어요. 청중에게 긴장된 인상을 줄 수 있어요. "
            f"발표 전 '이' 발음을 5초간 유지하거나 볼을 부풀렸다 터트리는 스트레칭을 해보세요. "
            f"얼굴 근육이 풀리면 자연스러운 표정이 나와요. 목표는 미소 비율 30% 이상이에요."
        )
