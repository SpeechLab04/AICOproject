...
def get_gesture_feedback(gesture_ratio, gesture_score):
    ...
    if neutral > 70:
        return (
            f"손 제스처를 사용한 비율이 {int(gesture_use)}%로 적어요. 발표가 다소 경직되어 보일 수 있어요. "
            f"양손 사이에 농구공을 들고 있다고 상상하며 설명해보세요. "
            f"그 자세에서 자연스럽게 손을 펼치거나 가리키면 어색하지 않은 제스처가 돼요. "
            f"목표는 손 사용 비율 30% 이상이에요."
        )
    elif gesture_score >= 80:
        return (
            f"손 제스처를 {int(gesture_use)}% 활용했어요. 발표 내용을 효과적으로 전달하고 있어요. "
            f"크고 명확한 제스처가 청중의 집중도를 높이고 있어요. 지금처럼 유지해봐요!"
        )
    elif gesture_score >= 65:
        return (
            f"손 제스처를 {int(gesture_use)}% 사용했어요. 적절해요. "
            f"핵심 내용을 강조할 때 양손을 더 크게 벌리거나, "
            f"농구공을 잡듯 두 손을 모아 앞으로 내미는 동작을 더해보세요. "
            f"목표는 손 사용 비율 50% 이상이에요."
        )
    elif pointing > 20:
        return (
            f"자료를 가리키는 제스처가 {int(pointing)}%로 많아요. "
            f"가리키는 것 외에도 손바닥을 펼쳐 설명하거나, "
            f"양손으로 크기나 범위를 표현하는 제스처를 함께 활용해보세요."
        )
    else:
        return (
            f"손 제스처를 사용한 비율이 {int(gesture_use)}%예요. 조금 더 과감하게 써보세요. "
            f"양손 사이에 농구공을 들고 있다고 상상하며 크게 움직여보세요. "
            f"작은 손짓보다 과감한 제스처가 발표의 설득력을 높여요. 목표는 30% 이상이에요."
        )
