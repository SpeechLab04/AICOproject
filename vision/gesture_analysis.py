...
def get_gesture_feedback(gesture_ratio, gesture_score):
    ...
    if neutral > 70:
        return (
            f"손 제스처를 사용한 비율이 {gesture_use}%로 적어요. 발표가 다소 경직되어 보일 수 있어요. "
        ...
        return (
            f"손 제스처를 {gesture_use}% 활용했어요. 발표 내용을 효과적으로 전달하고 있어요. "
            f"크고 명확한 제스처가 청중의 집중도를 높이고 있어요. 지금처럼 유지해봐요!"
        )
    elif gesture_score >= 65:
        return (
            f"손 제스처를 {gesture_use}% 사용했어요. 적절해요. "
        ...
        return (
            f"자료를 가리키는 제스처가 {pointing}%로 많아요. "
            f"가리키는 것 외에도 손바닥을 펼쳐 설명하거나, "
            f"양손으로 크기나 범위를 표현하는 제스처를 함께 활용해보세요."
        )
    else:
        return (
            f"손 제스처를 사용한 비율이 {gesture_use}%예요. 조금 더 과감하게 써보세요. "
