...
def get_gesture_feedback(gesture_ratio, gesture_score):
    ...
    if gesture_score is not None and gesture_score >= 80:
        return (
            f"손 제스처를 {int(gesture_use)}% 활용했어요. 크고 명확한 제스처가 청중의 집중도를 높이고 있어요. "
            f"지금처럼 유지해봐요!"
        )
    elif gesture_score is not None and gesture_score >= 65:
        return (
            f"손 제스처를 {int(gesture_use)}% 잘 활용했어요. "
            f"핵심 내용을 강조할 때 양손을 더 크게 벌리거나 "
            f"농구공을 잡듯 두 손을 모아 앞으로 내미는 동작을 더해보세요."
        )
    elif pointing > 40:
        return (
            f"자료를 가리키는 제스처가 {int(pointing)}%로 매우 많아요. "
            f"가리키는 동작 외에도 손바닥을 펼쳐 설명하거나 "
            f"양손으로 크기·범위를 표현하는 다양한 제스처를 활용해보세요."
        )
    elif pointing > 20:
        return (
            f"자료를 가리키는 제스처가 {int(pointing)}%로 있어요. "
            f"가리키는 것 외에도 손바닥을 펼쳐 설명하는 동작을 함께 써보세요. "
            f"제스처가 다양할수록 발표가 더 생동감 있어 보여요."
        )
    elif gesture_use >= 50:
        return (
            f"손 제스처를 {int(gesture_use)}% 많이 활용했어요. "
            f"핵심 내용을 말할 때 손을 가슴 높이로 올려 크게 움직여보세요. "
            f"손이 너무 낮으면 화면에 잘 안 보여요."
        )
    elif gesture_use >= 30:
        return (
            f"손 제스처를 {int(gesture_use)}% 활용했어요. 조금 더 과감하게 표현하면 좋겠어요. "
            f"말할 때 손을 가슴 높이로 올려 크게 움직여보세요. "
            f"손이 너무 낮으면 화면에 잘 안 보여요."
        )
    elif gesture_use >= 15:
        return (
            f"손 제스처가 {int(gesture_use)}%로 조금 있어요. 더 적극적으로 활용해보세요. "
            f"양손 사이에 농구공을 들고 있다고 상상하며 설명해보세요. "
            f"목표는 적극적 손동작 비율 30% 이상이에요."
        )
    elif gesture_use > 0:
        return (
            f"손 제스처가 {int(gesture_use)}%로 아주 적어요. 발표가 경직되어 보일 수 있어요. "
            f"양손을 자연스럽게 앞으로 내밀고 말하는 내용에 맞게 손을 펼치거나 가리키는 연습을 해보세요. "
            f"목표는 적극적 손동작 비율 30% 이상이에요."
        )
    else:
        return (
            f"손 제스처가 전혀 없어요. 발표가 딱딱하게 보일 수 있어요. "
            f"처음엔 강조할 단어마다 검지로 가볍게 가리키는 것부터 시작해보세요. "
            f"목표는 적극적 손동작(손펼침·가리킴·활발함) 비율 30% 이상이에요."
        )
